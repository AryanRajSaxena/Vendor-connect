...
export default function AddProductPage() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages = Array.from(files)
      .slice(0, 5 - images.length) // Max 5 images
      .map((file) => {
  ...
  const handlePublish = async (isDraft: boolean = false) => {
    try {
      ...
      if (isNaN(basePrice) || basePrice <= 0) {
        throw new Error('Base price must be a valid posit number');
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <Link href="/vendor/dashboard" className="text-primary hover:underline mb-6 inline-block">
          ← Back to Dashboard
        </Link>
            ...
            <div className="bg-white rounded-lg p-6 space-y-6">
              ...
              <div className="border-t pt-6">
                ...
                <div className="grid grid-cols-5 gap-3 mb-4">
                  {images.map((img, idx) => (
                    <div key={idx} className="relative w-24 h-24 bg-gray-100 rounded-lg flex items-center justify-center text-3xl">
                      {img}
                      <button
              ...
              <div className="border-t pt-6 flex gap-4">
                <button
                  ...
                  disabled={loading}
                  className="btn btn-outline flex-1 py-3 disabled:opacity-50"
                >
                  Save as Draft
                </button>
                <button
